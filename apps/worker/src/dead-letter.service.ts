...
export class DeadLetterService {
  ...
  async record(entry: DeadLetterIntegrationEventJob): Promise<void> {
    await this.pool.query(
      ...
      [randomUUID(), entry.event.eventId, entry.sourceJobId ?? null,
        entry.event.eventName, entry.event.eventVersion, this.consumerName,
        entry.reason, JSON.stringify(entry.event), entry.attempts, entry.failedAt],
    );
