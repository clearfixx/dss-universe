...
describe("processIntegrationEvent", () => {
  it("acknowledges a valid versioned job", async () => {
    ...
    const store: ProcessedEventStore = {
      hasProcessed: async (eventId, consumer) =>
        markers.has(`${eventId}:${consumer}`),
      markProcessed: async (event, consumer) =>
        void markers.add(`${event.eventId}:${consumer}`),
    };
    const handler = vi.fn(async () => undefined);
    const process = createIntegrationEventProcessor(
      store,
      handler,
      "test-consumer",
    );
    const data: IntegrationEventJob = {
      eventId: "event-1",
